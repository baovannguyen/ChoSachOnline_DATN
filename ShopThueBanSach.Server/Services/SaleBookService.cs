...
namespace ShopThueBanSach.Server.Services
{
    public class SaleBookService : ISaleBookService
    {
        ...
        public async Task<string> CreateAsync(CreateSaleBookDto dto)
        {
            ...
            saleBook.FinalPrice = await CalculateFinalPriceAsync(dto.Price, dto.PromotionId);
        ...
        public async Task<decimal> CalculateFinalPriceAsync(decimal basePrice, string? promotionId)
        {
            if (string.IsNullOrWhiteSpace(promotionId)) return basePrice;

            var promotion = await _context.Promotions.FindAsync(promotionId);
            if (promotion == null || promotion.StartDate > DateTime.UtcNow || promotion.EndDate < DateTime.UtcNow)
                return basePrice;

            return basePrice * (1 - (decimal)(promotion.DiscountPercentage / 100.0));
        }
